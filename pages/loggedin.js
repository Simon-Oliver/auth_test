...
const LoggedIn = () => {
  ...
  const loggAPI = async () => {
    try {
      ...
      const res = await firebase
        ...
        .collection("users")
        .doc(`${authUser.uid}`)
        .set({ someData: "123151afajk" }, { merge: true });
      const doc = await firebase
        .firestore()
        .collection("users")
        .doc(`${authUser.uid}`)
        .get();
      const resBox = await firebase
        .firestore()
        .collection("boxes")
        .doc(`${authUser.uid}`)
        .set(
  ...
  const onChangeHandler = (e) => {
    ...
    const index = userData.boxes
      .map(function (e) {
        return e.boxId;
      })
      .indexOf(data[0].boxId);
  ...
  const postBox = async () => {
    const resBox = await firebase
      ...
      .collection("boxes")
      .doc(`${authUser.uid}`)
      .set({ boxes: [...userData.boxes] }, { merge: true });
    const box = await firebase
      .firestore()
      .collection("boxes")
      .doc(`${authUser.uid}`)
                ...
                <Card {...{ title: e.name, content: e.content }}>
                  <div onClick={(e) => e.stopPropagation()}>
                    ...
                    <Container>
                      ...
                      <Button id={e.boxId} onClick={onDeleteClick} color="red">
                        delete
                      </Button>
                    </Container>
                  </div>
